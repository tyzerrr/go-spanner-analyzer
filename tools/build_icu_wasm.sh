...
echo "==> [3/3] libicudata.a を C 配列経由で作る（genccode は wasm の .o を扱えないため）"
DAT=$(ls "$OUT"/host/data/out/tmp/icudt*.dat | head -1); VER=$(basename "$DAT" .dat | sed -E "s/^icudt([0-9]+)[lb]$/\\1/")
